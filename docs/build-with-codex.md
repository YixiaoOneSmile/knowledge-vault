...
第二个是邮箱注册默认开启了邮箱验证，如果本地测试时没验证邮箱，登录会报 `400 email not confirmed`。解决方式是去 Supabase Dashboard 里关闭 Email confirmation，或者去邮箱点验证链接。
...
- Vercel：免费部署静态前端
- Supabase：免费额度足够原型验证
- Firecrawl：不是必需项，没有 key 也可以直接抓网页
- 大模型：使用已有代理 key，项目本身不绑定固定供应商
